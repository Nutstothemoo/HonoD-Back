...
import (
	"context"
	"fmt"
	"ginapp/internal/infrastructure/security"
...
func SignUp() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		token, refreshtoken, _ := security.TokenGenerator(*user.Email, *user.FirstName, *user.LastName, user.User_ID, *user.Role)
		user.Token = &token
...
func Login() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		tokenStr, refreshToken, _ := security.TokenGenerator(*founduser.Email, *founduser.FirstName, *founduser.LastName, founduser.User_ID, *founduser.Role)
		security.UpdateAllTokens(tokenStr, refreshToken, founduser.User_ID)
		http.SetCookie(c.Writer, &http.Cookie{
			Name:     "auth_token",
			Value:    tokenStr,
			MaxAge:   60 * 60 * 240,           // 10 days
			HttpOnly: true,                    // The cookie is not accessible via JavaScript
			Secure:   false,                   // The cookie is not sent only over HTTPS
			SameSite: http.SameSiteStrictMode, // The cookie is sent only to the same site as the one that originated it
		})
...
func RefreshToken() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		claims, err := security.VerifyRefreshToken(refreshToken)
		if err != nil {
		...
		newToken, newRefreshToken, err := security.TokenGenerator(*user.Email, *user.FirstName, *user.LastName, user.User_ID, *user.Role)
		if err != nil {
		...
		security.UpdateAllTokens(newToken, newRefreshToken, user.User_ID)
